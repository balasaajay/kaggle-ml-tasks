...
# 1. torch.save
torch.save('model', 'customer_sentiment.pt')
# To use pt file:
# restore_model = torch.load('customer_sentiment.pt')
# y_test_pred = restore_model(torch.from_numpy(sc.transform(np.array([[43, 60000]]))).float())

# 2. save model dictionary - Recommended way
# Creates dictionary with trained weights
torch.save(model.state_dict(), 'customer_sentiment_state_dict')
# To use dict:
# Download model dict file, pickle file
# new_pred = Net()
# new_pred.load_state_dict(torch.load("customer_sentiment_state_dict"))
# y_test_pred = new_pred(torch.from_numpy(sc.transform(np.array([[43, 60000]]))).float())
